-- Add special_guest column to social_attendance table
ALTER TABLE social_attendance 
ADD COLUMN IF NOT EXISTS special_guest BOOLEAN DEFAULT FALSE;

-- Update existing records to be non-special guests (paid)
UPDATE social_attendance 
SET special_guest = FALSE 
WHERE special_guest IS NULL;

-- Delete any existing special guest records to avoid conflicts
DELETE FROM social_attendance 
WHERE order_id >= 999001 AND order_id <= 999013;

-- Insert special guest data for June 14th event
INSERT INTO social_attendance (order_id, order_number, customer_id, customer_name, customer_email, social_date, social_name, total_tickets, tickets_used, special_guest) VALUES 
-- Special guests
